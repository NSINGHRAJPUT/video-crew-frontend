...
export default function AdminPortfolio() {
  ...
  const [formData, setFormData] = useState({
    ...
    thumbnail_url: '', video_url: '', featured: false, display_order: 1
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowForm(false);
      setEditItem(null);
      setFormData({ title: '', category: '', client: '', description: '', thumbnail_url: '', video_url: '', featured: false, display_order: 1 });
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Delete this item?')) {
      try {
        await fetch(`https://video-crew-backend-production.up.railway.app/api/portfolio/${id}`, {
          method: 'DELETE',
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {showForm && (
          <div className="bg-gray-900 p-6 rounded-lg mb-8">
            <h2 className="text-xl font-semibold mb-4">{editItem ? 'Edit' : 'Add'} Portfolio Item</h2>
        ...
        <div className="grid gap-4">
          {items.map((item) => (
            <div key={item._id} className="bg-gray-900 p-4 rounded-lg flex justify-between items-center">
              <div>
                <h3 className="font-semibold">{item.title}</h3>
                <p className="text-gray-400">{item.category} - {item.client}</p>
              </div>
              <div className="flex space-x-2">
                <button
